...
import { ImageGeneratorConfigTemplates, ImageGeneratorTemplates } from "../../config/image-generator";
...
const templateNames = ImageGeneratorTemplates.map(template => template.name) as [string, ...string[]];
...
type ImageGeneratorFormSchemaValues = zod.infer<typeof imageGeneratorFormSchema>;
...
export const ImageGeneratorForm = () => {
    const {
        ...
        handleSubmit,
        formState: { errors }
    } = useForm<ImageGeneratorFormSchemaValues>({ resolver: zodResolver(imageGeneratorFormSchema) });
    ...
    const  { mutate: imageGenerate, isPending } = useImageGenerate({ 
        onSuccess: (data) => {
            if (data.success) {
                setAPIStatus({ success: true, message: `${ data.payload.message } : ${ data.payload.images.length } images` });
            }
    ...
    const onSubmit = (values: ImageGeneratorFormSchemaValues) => {
        imageGenerate({
            ...
            properties: {
                ...
                ...(
                    ImageGeneratorConfigTemplates[values.template] || {}
    ...
    return (
        <Form className="image-generator-form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormFooter>
                ...
                {APIStatus && <QueriesStatus status={APIStatus.success ? 'success' : 'error'}>{APIStatus.message}</QueriesStatus> }
            </FormFooter>
        </Form>
    );
}